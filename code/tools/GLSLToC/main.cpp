	...
	for (int argIndex = 1; argIndex < argc; ++argIndex)
	{
		...
		if (*arg == '-')
		{
			...
			switch (arg[1])
			{
				...
				break;
				case 'm':
					outSettings->generateMonolithic = true;
				break;
				case 'f':
...
static void PrintHelp()
{
	...
	std::cout << "    -m: Generates a monolithic shader source file including all shaders." << std::endl;
	std::cout << std::endl;
}
...
static bool ShaderProgramToC(const std::string& outputName, EShLanguage stage, glslang::TProgram& prog, const std::vector<unsigned> &progDWords)
{
	...
	else
	{
		...
		else
		{
			...
			header << "#ifdef EXPORT" << std::endl;
			header << "# error \"'EXPORT' already defined.\"" << std::endl;
			header << "#endif //#ifdef EXPORT" << std::endl;
			header << std::endl;

			header << "#if BUILD_DLL" << std::endl;
			header << "# define EXPORT __declspec(dllexport)" << std::endl;
			header << "#elif USE_DLL //#if BUILD_DLL" << std::endl;
			header << "# define EXPORT __declspec(dllimport)" << std::endl;
			header << "#else //#elif USE_DLL //#if BUILD_DLL" << std::endl;
			header << "# define EXPORT" << std::endl;
			header << "#endif //#else //#elif USE_DLL //#if BUILD_DLL" << std::endl;
			header << std::endl;
			...
			header << "#if EXPORT_SHADER_SYM" << std::endl;
			header << "extern \"C\" EXPORT const char * const shader_sym;" << std::endl;
			header << "#endif //#if EXPORT_SHADER_SYM" << std::endl;
			header << std::endl;

			source << "#if EXPORT_SHADER_SYM" << std::endl;
			source << "extern \"C\" const char * const shader_sym = \"" << prefix << "shader\";" << std::endl;
			source << "#endif //#if EXPORT_SHADER_SYM" << std::endl;
			source << std::endl;

			header << "extern \"C\" EXPORT const ShaderModule " << prefix << "shader;" << std::endl;
			header << std::endl;

			source << "extern \"C\" const ShaderModule " << prefix << "shader = {" << std::endl;
...
int main(int argc, char *argv[])
{
	...
	for (size_t sourceIndex = 0; sourceIndex < settings.inputFiles.size(); ++sourceIndex)
	{
		...
		else
		{
			...
			{
				std::cout << "Building '" << source.fileName << "'." << std::endl;
	...
	{
		std::ofstream refHeader(settings.outputDir + "\\ShaderReflection.h");

		refHeader << "// Generated by GLSLToC. Do no modify." << std::endl;
		refHeader << std::endl;
		refHeader << "#pragma once" << std::endl;
		refHeader << std::endl;
		refHeader << "enum class ShaderStageType : unsigned char" << std::endl;
		refHeader << "{" << std::endl;
		refHeader << "	VERTEX," << std::endl;
		refHeader << "	TESS_CONTROL," << std::endl;
		refHeader << "	TESS_EVAL," << std::endl;
		refHeader << "	GEOMETRY," << std::endl;
		refHeader << "	FRAGMENT," << std::endl;
		refHeader << "	COMPUTE" << std::endl;
		refHeader << "};" << std::endl;
		refHeader << std::endl;
		refHeader << "//enum class GLSLBasicType : unsigned char" << std::endl;
		refHeader << "//{" << std::endl;
		refHeader << "//	BOOL," << std::endl;
		refHeader << "//	INT," << std::endl;
		refHeader << "//	FLOAT," << std::endl;
		refHeader << "//	SAMPLER_2D," << std::endl;
		refHeader << "//	SAMPLER_3D," << std::endl;
		refHeader << "//	ARRAY," << std::endl;
		refHeader << "//	STRUCT" << std::endl;
		refHeader << "//};" << std::endl;
		refHeader << std::endl;
		refHeader << "struct ShaderUniformBlock" << std::endl;
		refHeader << "{" << std::endl;
		refHeader << "	unsigned short binding;" << std::endl;
		refHeader << "	unsigned short sizeBytes;" << std::endl;
		refHeader << "};" << std::endl;
		refHeader << std::endl;
		refHeader << "//struct GLSLType" << std::endl;
		refHeader << "//{" << std::endl;
		refHeader << "//	GLSLBasicType basicType;" << std::endl;
		refHeader << "//	union" << std::endl;
		refHeader << "//	{" << std::endl;
		refHeader << "//		struct // mtx/vector/scalar" << std::endl;
		refHeader << "//		{" << std::endl;
		refHeader << "//			unsigned char rows : 4;" << std::endl;
		refHeader << "//			unsigned char cols : 4;" << std::endl;
		refHeader << "//		};" << std::endl;
		refHeader << "//		unsigned char elements; // array" << std::endl;
		refHeader << "//		unsigned char fields;   // struct" << std::endl;
		refHeader << "//	};" << std::endl;
		refHeader << "//};" << std::endl;
		refHeader << std::endl;
		refHeader << "struct ShaderModule" << std::endl;
		refHeader << "{" << std::endl;
		refHeader << "	const char* const name;" << std::endl;
		refHeader << "	const char* const entryPoint;" << std::endl;
		refHeader << "	const unsigned* const progam;" << std::endl;
		refHeader << "	const char* const* const uboNames;" << std::endl;
		refHeader << "	const ShaderUniformBlock* const ubos;" << std::endl;
		refHeader << "	//const char* const* const uniformNames;" << std::endl;
		refHeader << "	//const GLSLType* const uniforms;" << std::endl;
		refHeader << "	//const char* const* const inputNames;" << std::endl;
		refHeader << "	//GLSLType* const inputs;" << std::endl;
		refHeader << "	//const char* const* const outputNames;" << std::endl;
		refHeader << "	//GLSLType* const outputs;" << std::endl;
		refHeader << "	const unsigned programSizeDWords;" << std::endl;
		refHeader << "	ShaderStageType type;" << std::endl;
		refHeader << "	unsigned char uboCount;" << std::endl;
		refHeader << "	unsigned char inputCount;" << std::endl;
		refHeader << "	unsigned char outputCount;" << std::endl;
		refHeader << "};" << std::endl;

		if (settings.generateMonolithic)
		{
			...
			allHeader << "#ifdef EXPORT" << std::endl;
			allHeader << "# error \"'EXPORT' already defined.\"" << std::endl;
			allHeader << "#endif //#ifdef EXPORT" << std::endl;
			allHeader << std::endl;

			allHeader << "#if BUILD_DLL" << std::endl;
			allHeader << "# define EXPORT __declspec(dllexport)" << std::endl;
			allHeader << "#elif USE_DLL //#if BUILD_DLL" << std::endl;
			allHeader << "# define EXPORT __declspec(dllimport)" << std::endl;
			allHeader << "#else //#elif USE_DLL //#if BUILD_DLL" << std::endl;
			allHeader << "# define EXPORT" << std::endl;
			allHeader << "#endif //#else //#elif USE_DLL //#if BUILD_DLL" << std::endl;
			allHeader << std::endl;

			allHeader << "extern \"C\" EXPORT const unsigned shader_count;" << std::endl;
			allHeader << "extern \"C\" EXPORT const ShaderModule * const shaders[];" << std::endl;
			allHeader << std::endl;

			allHeader << "#undef EXPORT" << std::endl;
			...
			{
				allSource << "#include \"" << GetFilename(file.c_str()) << ".cpp\"" << std::endl;
			}
			allSource << std::endl;

			allSource << "extern \"C\" const unsigned shader_count = " << validPrograms.size() << ";" << std::endl;
			allSource << "extern \"C\" const ShaderModule * const shaders[] = {" << std::endl;
			for (const std::string& file : validPrograms)
			{
